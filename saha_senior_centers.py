...
if not os.path.exists('img'):
    os.makedirs('img')
# 다운로드 받은 default.jpg 파일을 img 폴더에 넣어주세요
...
def load_data(uploaded_file):
    ...
    if '일자' in df.columns:
        ...
        df['일자'] = pd.to_datetime(df['일자'], errors='coerce').dt.year  # 일자 열을 datetime으로 변환 후 연도로 변환
    if '사진경로' in df.columns:
        df['사진경로'] = df['사진경로'].fillna('default.jpg')  # NaN 값을 기본 이미지 경로로 대체
    return df
...
if uploaded_file is not None:
    ...
    # 사용자 정의 필터 추가
    st.sidebar.subheader("필터 추가")
    new_filter_column = st.sidebar.text_input("필터 컬럼명 추가")
    if 'filters' not in st.session_state:
        st.session_state['filters'] = []

    if st.sidebar.button("필터 추가"):
        if new_filter_column and new_filter_column not in st.session_state['filters'] and new_filter_column in df.columns:
    ...
    # 건축 연도 옵션
    if '일자' in df.columns:
        min_year = int(df['일자'].min())
        max_year = int(df['일자'].max())
        selected_year_range = st.sidebar.slider("건축 연도", min_year, max_year, (min_year, max_year))
        filtered_df = df[(df['일자'] >= selected_year_range[0]) & (df['일자'] <= selected_year_range[1])]
    else:
        filtered_df = df.copy()
    ...
    if not filtered_df.empty:
        ...
        for idx, row in filtered_df.iterrows():
            if not pd.isna(row['위도']) and not pd.isna(row['경도']):
                image_path = os.path.join(image_folder_path, row['사진경로'])
                if os.path.exists(image_path):
                    # 이미지 파일을 base64로 인코딩
                    with open(image_path, "rb") as img_file:
                        img_bytes = img_file.read()
                        img_b64 = base64.b64encode(img_bytes).decode("utf-8")
                        img_html = f'<img src="data:image/jpeg;base64,{img_b64}" alt="{row["시설명"]}" style="width: 100%; height: auto;">'
                    
                    # 팝업 HTML 설정 (사진이 있는 경우)
                    popup_html = f"""
                    ...
                    # 팝업 HTML 설정 (사진이 없는 경우 기본 이미지 표시)
