...
if 'uploaded_file' in st.session_state:
    ...
    for filter_col in st.session_state['filters']:
        ...
        unique_values.insert(0, '전체')
        # selected_values = st.sidebar.multiselect(f"{filter_col}", unique_values, default=unique_values)
        selected_values = st.sidebar.multiselect(f"{filter_col}", unique_values, default=['전체'])
        if '전체' in selected_values:
            selected_filters[filter_col] = unique_values[1:]  # '전체'를 제외한 모든 값을 선택
        else:
            selected_filters[filter_col] = selected_values

    # 건축 연도 옵션
    if '일자' in df.columns and '전체' not in st.session_state['filters']:
    ...
    # 필터링
    if '전체' not in st.session_state['filters']:
    ...
    default_popup_fields = [col for col in ["시설명", "주소"] if col in available_columns]
    popup_fields = st.sidebar.multiselect("팝업에 표시할 필드 선택", available_columns, default=default_popup_fields)
    ...
    if not filtered_df.empty:
        ...
        b64 = base64.b64encode(map_html_bytes).decode('utf-8')
        href = f'<a href="data:text/html;base64,{b64}" download="saha_senior_centers_map.html">지도 다운로드</a>'
